...
function urlsFromLink(url, depth, options) {
  ...
  return new Promise(function(resolve, reject) {
    if (depth == 0) { log.debug('Depth exceeded. This thread is dying.'); resolve(); }
    if (options.found.indexOf(url) > -1) { log.debug('Url was already parsed. This thread is dying.'); resolve(); }
    log.info('Scanning for urls:', url);
    _urlsFromLinkPromise(url, options)
      .then(function(urls) {
        var promises = _urlsFromLinkPromiseArray(urls, options);
        if (promises.length) {
 ...
 * @return {[type]}                            [description]
 */
function _urlsFromLinkPromise(url, options) {
  return new Promise(function(resolve, reject){
    child(
      url,
